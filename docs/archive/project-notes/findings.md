...
## 回忆图鉴体验升级调研（2026-05-31）
- 当前图鉴不是简单占位页：已经具有独立 `MemoryAlbum` 模型、章节、时间线、家庭补充问题、笔记和逐句朗读段落。
- `MemoryAlbumRepository` 会从老人档案、亲属、重要经历、日常生活、照片和视频中组合图鉴；视频会作为可展示媒体统一进入图鉴素材。
- 当前组合逻辑已具备“可以边看边听的回忆图鉴”目标：支持逐句朗读、当前句高亮、自动滚动或翻页、点击句子继续播放等叙事能力。
- 当前封面图选择顺序优先头像，其次重点照片，再其次普通照片。若头像偏证件照，封面可能显得像档案页，缺少回忆图鉴应有的情绪吸引力。
- 图鉴的产品亮点不应停留在“卡片列表展示资料”，更适合定位为“一本可听、可翻、可由家人继续补写的人生影集”。
- 初步推荐把“沉浸式可听影集”作为主体验，把“家庭补充问题”作为轻量辅助，把时间线作为二级导航；老人查看时保持低认知负担，家属浏览时仍能快速定位和补充。
- 生成式图片不宜替代真实家庭照片。若后续确认需要使用 `imagegen`，优先生成低干扰的纸张纹理、空状态插画或视觉方向预览素材。
- 当前 UI 有两个模式：默认 `_PhotoBookView` 是横向逐页翻看的照片墙；点击耳机后进入 `_MemoryViewMode.narration`，以纵向滚动方式展示档案卡、章节卡和家庭问题。
- 默认照片页已具备翻页动画、左右切换、页码、照片详情与事件详情，基础交互不错；但页面之间缺少章节、时间线和故事线索，老人看到的是“照片逐张翻看”，还没有形成“人生影集”的连续感。
- 朗读模式功能完整：逐句点击播放、当前句高亮、自动滚动、上一句/下一句、上一页/下一页、播放进度和语速均已存在；但视觉结构仍以 `_AlbumPanel` 统一卡片纵向堆叠为主，更像阅读资料页，照片与声音没有成为核心舞台。
- 关键设计机会：不要把“相册模式”和“听故事模式”设计成两套割裂页面。更适合把它们合并为同一本影集：真实照片负责情绪入口，章节叙事负责连续性，底部朗读控件负责随时播放，家庭补充问题在章节结束后轻量出现。
- 用户已确认采用方向 A“可听的人生影集”作为主体，并吸收方向 B“人生时间轴”和方向 C“家庭共编档案”的优点。
- 推荐的信息架构因此收敛为：沉浸式影集主页面 + 章节内时间定位或二级时间线入口 + 章节末尾轻量家庭补充提示。避免把时间线和资料补全直接放成第一屏主角。
- 用户确认核心受众优先级：第一优先服务老人本人。界面应优先保证大照片、少操作、清晰的一键朗读与低认知负担；家属补充资料和展示型信息作为次级入口。
- 用户确认保留封面仪式感：打开“回忆图鉴”后先展示一本影集封面，再由老人点击明显的“开始听故事”按钮进入章节阅读，而不是直接落入第一张照片。
- 用户确认调整封面照片策略：优先使用家属手动标记的重点照片，其次选择合适的家庭照片，最后才回退老人头像。现有头像优先逻辑需要在实施阶段调整。
- 用户确认家庭共编首版采用轻量方案：章节末尾展示“还有 N 个故事可以补充”等提示，点击后跳转现有数据预录入模块；不在图鉴内部重复实现一套编辑表单。

## 回忆图鉴体验升级实施结果（2026-05-31）
- 图鉴主路径已经从旧的照片墙切换为三层体验：影集封面、统一阅读页、人生时间线。
- 封面选择顺序已经调整为：重点非头像照片、家庭照片、其他非头像照片、头像兜底。新增 3 条仓库测试覆盖这一顺序。
- 影集封面使用真实推荐照片，并提供“开始听故事”和“先翻一翻”：前者进入阅读页并启动已有逐句朗读，后者只进入阅读页。
- 统一阅读页复用已有 `NarrationPlayer`、逐句高亮、自动滚动和底部播放控制条；新增横向章节条和时间线入口。
- 章节内容弱化统一卡片堆叠，改为照片优先的故事段落；章节内补充问题以轻量提示呈现。
- 新增人生时间线页：展示时间、标题、内容和关联照片，并提供跳转数据预录入的补充入口。
- 数据预录入页面新增返回来源记忆：从设置进入时返回设置，从图鉴进入时返回图鉴。
- 用户在图鉴验收阶段进一步确认默认朗读音色需要使用 `wanqing`。Flutter Provider、Flutter TTS 仓库接口、Python TTS 默认值、测试和 README 已统一改为 `wanqing`；显式传入 `yunye` 的底层协议测试继续保留，用于验证指定音色覆盖默认值。
- 实际 Flutter Web 产物已在 `http://localhost:8092` 提供预览；入口、`flutter_bootstrap.js` 与 `main.dart.js` 均返回 HTTP 200。内置浏览器自动控制仍持续超时，本机也没有备用 Playwright 模块，因此截图级自动验收仍需人工查看页面。

## 回忆图鉴验收反馈后的简化（2026-05-31）
- 用户确认上一版阅读页信息密度过高，且图鉴不需要任何“家里人补充”提示。时间线末尾、章节末尾和旧条目组件中的相关提示均已移除。
- 图鉴标题下的副标题不再由故乡、经历数、亲友数和照片数拼接，统一固定为“慢慢翻，也慢慢听”。
- 新增 `MemoryAlbumStoryPage` 映射层：只把 `life_experience`、`photo_memory` 和 `daily_life` 章节送入老人阅读页；档案和亲属资料不再混入主阅读体验。
- 每一页正文最多保留两句。页面本身只展示一张大图、章节名、标题、可选时间地点和短正文。
- 缺少条目配图时使用推荐封面图兜底；仍没有可用照片时展示克制的空图片占位。
- 图鉴朗读不再使用独立的系统 `NarrationPlayer` 主路径，改为调用聊天“陪伴”页同一个 `VoiceOutputProvider.toggleReadAloud`。因此音色统一为 `wanqing`，并沿用设置页语速、音量和 WAV 缓存。
- 图鉴不会自动朗读，也不会连续播放整本影集。只有点击“朗读当前故事”后才合成并播放；翻页、退出阅读页、刷新或打开时间线都会停止播放。
- 交付前验证：Python TTS 8 项通过；Flutter 全量 55 项通过；Windows Debug 与 Web Debug 构建通过；`http://localhost:8092/`、`flutter_bootstrap.js` 和 `main.dart.js` 均返回 HTTP 200。
- `dart analyze` 没有编译错误，但仍返回 14 条提示：11 条为仓库既有提示，3 条来自替换后保留的旧版不可达朗读组件。旧组件不再进入图鉴运行路径，后续可单独做清理。
- 内置浏览器自动接管连续两次超时，因此本轮仍无法完成截图级验收；需要人工打开 `http://localhost:8092/` 检查真实视觉效果。
