"""
情绪安抚 — 最高优先级
优先级: 1（覆盖日常问候、往事深挖、认知干预）

数据库:
- 读 topic_preferences / memory_events: 取老人偏好的开心话题、正向记忆摘要，用于温柔转场
- 可写 conversation_logs 标记 emotion_incident（由应用层决定）
"""
...
def get_prompt(
    ...
    **_: Any,
    ...
    """
    emotion_type: sad | anxious | angry | pain 等
    positive_topics: 从 topic_preferences(偏好) 与 memory_events(正向片段) 合并、去敏后传入
    """
    pos = _lines(
        positive_topics,
